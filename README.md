...
In a half baked attempt to make a full stack application that simulates the functionality of a popular online auction site, EBay, an amateur web-developer set out on GitHub to start building. Unfortunately, this developer was not a Hack Reactor grad and was not proficient in building full-stack applications. He has now come to you through LinkedIn to ask you for help with the following specifications:
  ...
  - Must have all routes functional
    - [X] make sure to write/fix the routes and queries such that a postman request will still perform the desired action
    - [X] minimally, the get and update requests must work from the front-end so that we can get products rendered on our page from the back-end and bid on specific items (post and delete should still work from postman)
...
Being the amazing engineer you are, you break down the provided specs into the following steps:
  ...
  6. Let's go a few steps further. Refactor your code to implement the following
    ...
    - [X] search - implement a search functionality so that a product you search for is rendered into the product view (you may take creative liberties for how your search algorithm works. hint: filter?)
    - [X] add a button that on click, a user should be able to add a new product to the database
...
To next developer: Do these things:
  = Run "npm install" to install all dependencies.
  = Run "npm start" to start up the server.
  = Seed the data by running "node seed.js" from within the db folder of the repo.
  = Navigate to localhost:3000 to reach the application.
  = Have fun!!
